...
import { baseURL, teacher_listURL } from '../../config/AppData'
import axios from 'axios'
import TableLoader from '../../components/TableLoader'
const Teachers = ({ icon, name }) => {
        ...
        <div className="row data-op-row">
          ...
          <span className='col-md-6 data-op'>
            <div className="row justify-content-evenly align-items-center">
              ...
              <div className="col-6 ">
                <Link to={_routes.app.urls.add_teacher} className="btn btn-primary ">
        ...
        <tbody>
          {
            teachers.length !== 0 ? (
              teachers.map((item, index) => (
                <tr key={index} >
                  <td>{index + 1}</td>
                  <td>{item.teacher_details.name}</td>
                  <td>{item.teacher_details.phone}</td>
                  <td>{item.email}</td>
                  <td>{item.teacher_details.designation}</td>
                  <td  >
